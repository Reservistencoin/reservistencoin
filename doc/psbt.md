# PSBT Howto for ReservistenCoin Core

Since ReservistenCoin Core 0.17, an RPC interface exists for Partially Signed ReservistenCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/reservistencoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for ReservistenCoin transactions that are not fully signed
...
#### Multisig with multiple ReservistenCoin Core instances
...
ReservistenCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `reservistencoin-cli` is used.
...
Later, when *V* RVC has been received on *Amulti*, and Bob and Carol want to
