# Fuzzing ReservistenCoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing ReservistenCoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/reservistencoin/reservistencoin
$ cd reservistencoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/reservistencoin/reservistencoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of ReservistenCoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/reservistencoin_report.pdf) includes an external overview of the status of ReservistenCoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/reservistencoin/reservistencoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/reservistencoin/reservistencoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/reservistencoin/reservistencoin/tree/master/src/test/fuzz).
...
It is possible to specify `reservistencoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`reservistencoin-core/qa-assets`](https://github.com/reservistencoin-core/qa-assets) repo.

To fuzz `process_message` using the [`reservistencoin-core/qa-assets`](https://github.com/reservistencoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/reservistencoin-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/reservistencoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`reservistencoin-core/qa-assets`](https://github.com/reservistencoin-core/qa-assets) repo.

Every single pull request submitted against the ReservistenCoin Core repo is automatically tested against all inputs in the [`reservistencoin-core/qa-assets`](https://github.com/reservistencoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make ReservistenCoin Core more robust.
...
with certain assembly code from ReservistenCoin Core's code. See [developer notes on sanitizers](https://github.com/reservistencoin/reservistencoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing ReservistenCoin Core using afl++

## Quickstart guide

To quickly get started fuzzing ReservistenCoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing ReservistenCoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing ReservistenCoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the ReservistenCoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as ReservistenCoin
Core without having to write any custom fuzzing harness. The `reservistencoind` server
...
```sh
$ mkdir reservistencoin-honggfuzz-p2p/
$ cd reservistencoin-honggfuzz-p2p/
$ git clone https://github.com/reservistencoin/reservistencoin
$ cd reservistencoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/reservistencoind.cpp b/src/reservistencoind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/reservistencoind.cpp
+++ b/src/reservistencoind.cpp
...
EOF
$ make -C src/ reservistencoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/reservistencoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing ReservistenCoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing ReservistenCoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/reservistencoin/reservistencoin/pull/22472) for more information.
...
ReservistenCoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/reservistencoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=reservistencoin-core).
...
because ReservistenCoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_reservistencoin-core/latest).
