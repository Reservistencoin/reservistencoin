# CJDNS support in ReservistenCoin Core

It is possible to run ReservistenCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of ReservistenCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ReservistenCoin network and individual nodes.
...
before it will work with your ReservistenCoin Core node. This is described in steps
...
## Run ReservistenCoin Core with CJDNS

Once you are connected to the CJDNS network, the following ReservistenCoin Core
...
When enabled, this option tells ReservistenCoin Core that it is running in an
...
IPv6 local network. This helps ReservistenCoin Core perform better address management:
...
CJDNS support was added to ReservistenCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `reservistencoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/reservistencoin/reservistencoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in ReservistenCoin Core

There are several ways to see your CJDNS address in ReservistenCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `reservistencoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `reservistencoin-cli getpeerinfo`).
