# User-space, Statically Defined Tracing (USDT) for ReservistenCoin Core

ReservistenCoin Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ reservistencoind     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: ReservistenCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to ReservistenCoin Core
...
Multiple tools can list the available tracepoints in a `reservistencoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in ReservistenCoin Core, use `info probes` in `gdb`:

```
$ gdb ./src/reservistencoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/reservistencoind
stap net        outbound_message 0x0000000000107c05 /src/reservistencoind
stap validation block_connected  0x00000000002fb10c /src/reservistencoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in ReservistenCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/reservistencoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/reservistencoind -v
