ReservistenCoin Core version 0.11.0 is now available from:

  <https://reservistencoin.org/bin/reservistencoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer RVC/kB fee to be
...
For example, add the following to `reservistencoin.conf`:
...
related to the blockchain in the reservistencoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows ReservistenCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that ReservistenCoin Core will 
...
For an example script see [reservistencoin-submittx](https://github.com/laanwj/reservistencoin-submittx).
...
- #5334 `cf87536` libreservistencoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new reservistencoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ReservistenCoinAmountField
- #5753 `0f44672` Add reservistencoin logo to about screen
...
- #5809 `46bfbe7` Add reservistencoin-cli man page
...
- #5849 `41259ca` contrib/init/reservistencoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename ReservistenCoin-Qt.app to ReservistenCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for reservistencoin-tx)
