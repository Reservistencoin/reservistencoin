ReservistenCoin Core version 0.13.1 is now available from:

  <https://reservistencoin.org/bin/reservistencoin-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming ReservistenCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for ReservistenCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each ReservistenCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of reservistencoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of reservistencoins being spent is exactly
...
- **Increased security for multisig:** ReservistenCoin addresses (both P2PKH addresses
  ...
  believed to be provided by ReservistenCoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ReservistenCoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of ReservistenCoin's current ledger (called the UTXO set),
  ...
  note that ReservistenCoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ReservistenCoin users to individually opt-in to almost any change in the ReservistenCoin
  Script language when those users receive new transactions.  Features
  currently being researched by ReservistenCoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other ReservistenCoin community members are studying several other improvements
...
[BIP141]: https://github.com/reservistencoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/reservistencoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/reservistencoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/reservistencoin/bips/blob/master/bip-0145.mediawiki
...
Since ReservistenCoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/reservistencoin/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in reservistencoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.reservistencoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libreservistencoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_reservistencoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add reservistencoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` reservistencoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_reservistencoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name reservistencoin.qrc  (reservistencoinsSG)
...
- #8817 `bcf3806` update reservistencoin-tx to output witness data (jnewbery)
