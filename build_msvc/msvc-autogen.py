...
libs = [
    'libreservistencoin_cli',
    'libreservistencoin_common',
    'libreservistencoin_crypto',
    'libreservistencoin_node',
    'libreservistencoin_util',
    'libreservistencoin_wallet_tool',
    'libreservistencoin_wallet',
    'libreservistencoin_zmq',
    'bench_reservistencoin',
    'libtest_util',
]
...
def parse_config_into_rvc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"ReservistenCoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/reservistencoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/reservistencoin_config.h'), "w", encoding="utf8") as rvc_config:
        rvc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='ReservistenCoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_rvc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/reservistencoin_config.h'), os.path.join(SOURCE_DIR, 'config/reservistencoin-config.h'))
