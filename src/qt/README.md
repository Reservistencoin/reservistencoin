This directory contains the source code for the ReservistenCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### reservistencoingui.(h/cpp)

- Represents the main window of the ReservistenCoin UI.
...
- ClientModel is used by the main application `reservistencoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a reservistencoin:... link from a browser).
...
* UI elements like ReservistenCoinAmountField, which inherit from QWidget.
* `reservistencoinstrings.cpp`: automatically generated
* `reservistencoinunits.(h/cpp)`: RVC / mRVC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/reservistencoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `reservistencoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `reservistencoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `reservistencoin-qt`
9. You're all set! Start developing, building, and debugging the ReservistenCoin Core GUI
