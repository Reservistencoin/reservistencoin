...
#include <config/reservistencoin-config.h>
#endif

#include <qt/reservistencoin.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ReservistenCoinUnit>("ReservistenCoinUnit");
#else
    qRegisterMetaType<ReservistenCoinUnit>("ReservistenCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. reservistencoin_de.qm (shortcut "de" needs to be defined in reservistencoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. reservistencoin_de_DE.qm (shortcut "de_DE" needs to be defined in reservistencoin.qrc)
...
static const char* qt_argv = "reservistencoin-qt";

ReservistenCoinApplication::ReservistenCoinApplication()
...
void ReservistenCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ReservistenCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ReservistenCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ReservistenCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ReservistenCoinApplication::createOptionsModel(bool resetSettings)
{
...
void ReservistenCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ReservistenCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ReservistenCoinGUI::quitRequested, this, &ReservistenCoinApplication::requestShutdown);
...
void ReservistenCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ReservistenCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ReservistenCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ReservistenCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ReservistenCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ReservistenCoinApplication::handleRunawayException);
    connect(this, &ReservistenCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ReservistenCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ReservistenCoinApplication::parameterSetup()
{
...
void ReservistenCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ReservistenCoinApplication::requestInitialize()
{
...
void ReservistenCoinApplication::requestShutdown()
{
...
void ReservistenCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // reservistencoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ReservistenCoinGUI::handlePaymentRequest);
            connect(window, &ReservistenCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ReservistenCoinApplication::handleRunawayException(const QString &message)
{
...
void ReservistenCoinApplication::handleNonFatalException(const QString& message)
{
...
WId ReservistenCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ReservistenCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ReservistenCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(reservistencoin);
    Q_INIT_RESOURCE(reservistencoin_locale);
    ...
    ReservistenCoinApplication app;
    ...
    /// 6-7. Parse reservistencoin.conf, determine network, switch to network specific
    ...
    // reservistencoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
