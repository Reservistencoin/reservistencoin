...
#include <qt/reservistencoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ReservistenCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ReservistenCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ReservistenCoinAddressCheckValidator(parent));
}
...
bool parseReservistenCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no reservistencoin: URI
    if(!uri.isValid() || uri.scheme() != QString("reservistencoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ReservistenCoinUnits::parse(ReservistenCoinUnit::RVC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseReservistenCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseReservistenCoinURI(uriInstance, out);
}

QString formatReservistenCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("reservistencoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ReservistenCoinUnits::format(ReservistenCoinUnit::RVC, info.amount, false, ReservistenCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openReservistenCoinConf()
{
    ...
    /* Open reservistencoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ReservistenCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ReservistenCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("ReservistenCoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for ReservistenCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "reservistencoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("reservistencoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a reservistencoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=ReservistenCoin\n";
        else
            optionFile << strprintf("Name=ReservistenCoin (%s)\n", chain);
