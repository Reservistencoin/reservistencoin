...
/** Call node.updateRwSetting() with ReservistenCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because ReservistenCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/reservistencoin/reservistencoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because reservistencoin 22.x will not try to read these.
...
//! Convert enabled/size values to reservistencoin -prune setting.
...
//! Get pruning enabled value to show in GUI from reservistencoin -prune setting.
...
//! Get pruning size value to show in GUI from reservistencoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayReservistenCoinUnit")) {
        settings.setValue("DisplayReservistenCoinUnit", QVariant::fromValue(ReservistenCoinUnit::RVC));
    }
    QVariant unit = settings.value("DisplayReservistenCoinUnit");
    if (unit.canConvert<ReservistenCoinUnit>()) {
        m_display_reservistencoin_unit = unit.value<ReservistenCoinUnit>();
    } else {
        m_display_reservistencoin_unit = ReservistenCoinUnit::RVC;
        settings.setValue("DisplayReservistenCoinUnit", QVariant::fromValue(m_display_reservistencoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if reservistencoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_reservistencoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ReservistenCoinUnit>() == m_display_reservistencoin_unit) return;
    m_display_reservistencoin_unit = new_unit.value<ReservistenCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayReservistenCoinUnit", QVariant::fromValue(m_display_reservistencoin_unit));
    Q_EMIT displayUnitChanged(m_display_reservistencoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/reservistencoin/reservistencoin/pull/8273
    ...
    // (https://github.com/reservistencoin-core/gui/issues/567).
