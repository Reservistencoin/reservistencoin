...
static void SetupReservistenCoinTxArgs(ArgsManager &argsman)
{
...
{
    SetupReservistenCoinTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " reservistencoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  reservistencoin-tx [options] <hex-tx> [commands]  Update hex-encoded reservistencoin transaction\n"
                "or:     reservistencoin-tx [options] -create [commands]   Create hex-encoded reservistencoin transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded reservistencoin transaction
