...
const char * const RESERVISTENCOIN_CONF_FILENAME = "reservistencoin.conf";
const char * const RESERVISTENCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //reservistencoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", RESERVISTENCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\ReservistenCoin
    // macOS: ~/Library/Application Support/ReservistenCoin
    // Unix-like: ~/.reservistencoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ReservistenCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ReservistenCoin";
#else
    // Unix-like
    return pathRet / ".reservistencoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", RESERVISTENCOIN_CONF_FILENAME));
}
