...
TMPDIR_PREFIX = "reservistencoin_func_test_"
...
class ReservistenCoinTestMetaClass(type):
    """Metaclass for ReservistenCoinTestFramework.

    Ensures that any attempt to register a subclass of `ReservistenCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ReservistenCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ReservistenCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ReservistenCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ReservistenCoinTestFramework(metaclass=ReservistenCoinTestMetaClass):
    """Base class for a reservistencoin test script.

    Individual reservistencoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave reservistencoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop reservistencoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use reservistencoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_reservistencoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "reservistencoind" + config["environment"]["EXEEXT"],
        )
        fname_reservistencoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "reservistencoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_reservistencoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "reservistencoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.reservistencoind = os.getenv("RESERVISTENCOIND", default=fname_reservistencoind)
        self.options.reservistencoincli = os.getenv("RESERVISTENCOINCLI", default=fname_reservistencoincli)
        self.options.reservistencoinutil = os.getenv("RESERVISTENCOINUTIL", default=fname_reservistencoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: reservistencoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ReservistenCoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'reservistencoind', self.options.reservistencoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'reservistencoin-cli', self.options.reservistencoincli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                reservistencoind=binary[i],
                reservistencoin_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple reservistencoinds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a reservistencoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple reservistencoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as reservistencoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    reservistencoind=self.options.reservistencoind,
                    reservistencoin_cli=self.options.reservistencoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in reservistencoin.conf
    ...
    def skip_if_no_reservistencoind_tracepoints(self):
        """Skip the running test if reservistencoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("reservistencoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_reservistencoind_zmq(self):
        """Skip the running test if reservistencoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("reservistencoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if reservistencoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("reservistencoin-wallet has not been compiled")

    def skip_if_no_reservistencoin_util(self):
        """Skip the running test if reservistencoin-util has not been compiled."""
        if not self.is_reservistencoin_util_compiled():
            raise SkipTest("reservistencoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if reservistencoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("reservistencoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether reservistencoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether reservistencoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_reservistencoin_util_compiled(self):
        """Checks whether reservistencoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_RESERVISTENCOIN_UTIL")
