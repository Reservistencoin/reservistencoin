...
dnl Output: If qt version is auto, set reservistencoin_enable_qt to false. Else, exit.
AC_DEFUN([RESERVISTENCOIN_QT_FAIL],[
  if test "$reservistencoin_qt_want_version" = "auto" && test "$reservistencoin_qt_force" != "yes"; then
    if test "$reservistencoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; reservistencoin-qt frontend will not be built])
    fi
    reservistencoin_enable_qt=no
    reservistencoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([RESERVISTENCOIN_QT_CHECK],[
  if test "$reservistencoin_enable_qt" != "no" && test "$reservistencoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
dnl RESERVISTENCOIN_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([RESERVISTENCOIN_QT_PATH_PROGS],[
  RESERVISTENCOIN_QT_CHECK([
    if test "$3" != ""; then
      AC_PATH_PROGS([$1], [$2], [], [$3])
    else
      AC_PATH_PROGS([$1], [$2])
    ...
    if test "$$1" = "" && test "$4" != "yes"; then
      RESERVISTENCOIN_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other RESERVISTENCOIN_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([RESERVISTENCOIN_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build reservistencoin-qt GUI (default=auto)])],
    [
     reservistencoin_qt_want_version=$withval
     if test "$reservistencoin_qt_want_version" = "yes"; then
       reservistencoin_qt_force=yes
       reservistencoin_qt_want_version=auto
     fi
    ],
    [reservistencoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); reservistencoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   RESERVISTENCOIN_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _RESERVISTENCOIN_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: reservistencoin_enable_qt, reservistencoin_enable_qt_dbus, reservistencoin_enable_qt_test
AC_DEFUN([RESERVISTENCOIN_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  RESERVISTENCOIN_QT_CHECK([_RESERVISTENCOIN_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN does
  ...
  _RESERVISTENCOIN_QT_IS_STATIC
  if test "$reservistencoin_cv_static_qt" = "yes"; then
    _RESERVISTENCOIN_QT_CHECK_STATIC_LIBS
    ...
    if test "$TARGET_OS" != "android"; then
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "$TARGET_OS" = "windows"; then
      ...
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      AC_DEFINE([QT_QPA_PLATFORM_WINDOWS], [1], [Define this symbol if the qt platform is windows])
    elif test "$TARGET_OS" = "linux"; then
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  RESERVISTENCOIN_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  RESERVISTENCOIN_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  RESERVISTENCOIN_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  RESERVISTENCOIN_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  RESERVISTENCOIN_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  RESERVISTENCOIN_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  RESERVISTENCOIN_QT_CHECK([
    reservistencoin_enable_qt=yes
    reservistencoin_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      reservistencoin_enable_qt_test=no
    fi
    reservistencoin_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      reservistencoin_enable_qt_dbus=yes
    fi
  ...
  if test $reservistencoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$reservistencoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$reservistencoin_enable_qt])
  fi
...
dnl
dnl _RESERVISTENCOIN_QT_IS_STATIC
dnl ---------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
dnl Output: reservistencoin_cv_static_qt=yes|no
AC_DEFUN([_RESERVISTENCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, reservistencoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [reservistencoin_cv_static_qt=yes],
      [reservistencoin_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_RESERVISTENCOIN_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); RESERVISTENCOIN_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _RESERVISTENCOIN_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_RESERVISTENCOIN_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _RESERVISTENCOIN_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_RESERVISTENCOIN_QT_FIND_LIBS],[
  RESERVISTENCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [RESERVISTENCOIN_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  RESERVISTENCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [RESERVISTENCOIN_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  RESERVISTENCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [RESERVISTENCOIN_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  RESERVISTENCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [RESERVISTENCOIN_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
