# Bootstrappable ReservistenCoin Core Builds

This directory contains the files necessary to perform bootstrappable ReservistenCoin
Core builds.
...
To build ReservistenCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      ReservistenCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/reservistencoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/reservistencoin-core/guix.sigs).
