...
""" Example logging ReservistenCoin Core mempool events using the mempool:added,
...
def main(reservistencoind_path):
    reservistencoind_with_usdts = USDT(path=str(reservistencoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    reservistencoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    reservistencoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    reservistencoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    reservistencoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[reservistencoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/reservistencoind")
        exit(1)
