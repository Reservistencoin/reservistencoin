...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing reservistencoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='reservistencoin-cli', help='reservistencoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='ReservistenCoin address to which the faucet should send')
...
parser.add_argument('reservistencoin_cli_args', nargs='*', help='Arguments to pass on to reservistencoin-cli (default: -signet)')

args = parser.parse_args()

if args.reservistencoin_cli_args == []:
    args.reservistencoin_cli_args = ['-signet']


def reservistencoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.reservistencoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = reservistencoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = reservistencoin_cli(['getnewaddress', 'faucet', 'bech32'])
